...
func (t *TopologyMap) Init(config TopologyMapConfig) {
	...
	t.TopologyExpire = 15000
	if config.Topology_expire != 0 {
		t.TopologyExpire = config.TopologyExpire * 1000 // millisec
	}

}

func (t *TopologyMap) randomClient() *elasticsearch.Client {
	switch len(t.clients) {
	case 0:
		return nil
	case 1:
		return t.clients[0].(*elasticsearch.Client).Clone()
	default:
		return t.clients[rand.Intn(len(t.clients))].(*elasticsearch.Client).Clone()
	}
}

// Enable using ttl as paramters in a server-ip doc type
func (t *TopologyMap) EnableTTL() error {
...
func (t *TopologyMap) GetNameByIP(ip string) string {
